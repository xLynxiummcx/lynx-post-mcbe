...
vec3 SampleChromatic(vec2 uv, float offset) {
    vec2 dir = uv - 0.5;
    vec3 col;
    col.r = texture(s_ColorTexture, uv + dir * offset).r;
    col.g = texture(s_ColorTexture, uv).g;
    col.b = texture(s_ColorTexture, uv - dir * offset).b;
...
float ComputeAutoExposure(vec2 uv) {
    float avgLum = texture(s_AverageLuminance, vec2(0.5)).r;
    avgLum = max(avgLum, 0.0001);

    float targetLum = 0.18;
    float targetExposure = targetLum / avgLum;

    float prevExposure = texture(s_PreExposureLuminance, vec2(0.5)).r;
...
float AutoBlurStrength(vec2 uv) {
    ...
    float prevBlur = texture(s_PreExposureLuminance, vec2(0.5)).r;
